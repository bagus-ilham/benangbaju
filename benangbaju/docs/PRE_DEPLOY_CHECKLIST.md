...
- [ ] `05_ledger_hotfix.sql` dan `06_vault_compliance.sql` (bila menggunakan eksekusi manual) telah di-*run* dengan aman tanpa error.
...
Pastikan semua *environment variables* berikut terisi dengan nilai *production* di platform *hosting* (misal: Vercel):
...
- [ ] `NEXT_PUBLIC_APP_URL` (Ganti ke domain asli aplikasi, misal `https://benangbaju.com`)
...
- [ ] Jalankan `npm run lint` dan pastikan tidak ada *error*.
- [ ] Jalankan `npm run test` dan pastikan seluruh *unit test* (38 tests) PASS.
