...
export async function getAdminCustomersAction(page = 1, limit = 20) {
  ...
  const { adminGetCustomers } = await import('@/modules/users/adminCustomer.repository')
...
export async function toggleAdminCustomerStatusAction(customerId: string, isActive: boolean) {
  await requireAdmin()
  const { createAdminClient } = await import('@/lib/supabase/admin')
  const { adminToggleCustomerStatus } = await import('@/modules/users/adminCustomer.repository')
...
export async function getAdminCustomerDetailAction(customerId: string) {
  await requireAdmin()
  const { createAdminClient } = await import('@/lib/supabase/admin')
  const { adminGetCustomerDetail } = await import('@/modules/users/adminCustomer.repository')
