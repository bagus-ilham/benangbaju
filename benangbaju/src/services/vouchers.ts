...
import * as types from "@/modules/voucher/domain/voucher.types";

export type { VoucherValidationResult } from "@/modules/voucher/domain/voucher.types";

export async function validateVoucher(supabase: SupabaseClient<Database>, code: string, subtotal: number, userId: string) {
...
export async function adminUpdateVoucher(supabase: SupabaseClient<Database>, voucherId: string, voucherData: {
        code: string
        name: string
        discount_type: 'percentage' | 'fixed'
        value: number
